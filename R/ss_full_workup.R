...
ss_full_workup <- function(geLoc,ssLoc,dirklen,satflashlen,timeoffset,graph=F,remake=F,baseline=F){
  ...
  ecsdat <- ssdat$ecs %>%
    lapply(ss_bookkeeping) %>% 
    lapply(function(x)splitfun(x,dirklen)) %>% 
    lapply(function(x) ss_ecs_fit_all(x,graph=graph,remake=remake,abs520=baseline))
  if(graph){
  ...
  phi2dat <- ssdat$ps2 %>% 
    lapply(ss_bookkeeping) %>% 
    lapply(function(x) splitfun(x,satflashlen)) %>% 
    lapply(function(x) ss_phi2_calculation(x,graph=graph))
  if(graph){
    phi2graphs <- lapply(phi2dat,function(x) x[[2]])
    phi2dat <- lapply(phi2dat, function(x) x[[1]])
  }
  phi2dat <- rbind_all(phi2dat)
  #probably not saturating - be advised
  phi2dat %<>% select(-Fm,-Fs)
  alldat <- bind_rows(
    lapply(
      X=list(gedat,fitdat,phi2dat),
      FUN= function(x) melt(x,id.vars="Time")
    )
  )
  myplot <- ggplot(alldat,mapping=aes(x=Time,y=value,col=variable))+
    geom_point()+
